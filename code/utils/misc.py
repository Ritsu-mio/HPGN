...
def save_model_no_delete(model, when, step, ckpt_dir, states):
    model_tpl = "model={model}-weights-step={step}.pth"
    torch.save(states, join(ckpt_dir, model_tpl.format(model=model, step=step)))
